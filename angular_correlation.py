...
def main():
    ...
    for condition, marker in zip(args.conditions, args.marker_list):
        ...
        for row_idx, (row_label, comp_key, y_lbl) in enumerate(rows_titles):
            ...
            if flow_list:
                df_f = make_correlation_df(flow_list, var_name='flow_corr')
            ...
            # 2. Bead-flow correlation
            bead_list = data[f'bead_{comp_key}']
            if bead_list:
                df_b = make_correlation_df(bead_list, var_name='bead_corr')
            ...
            # 3. Background flow correlation
            bg_list = data[f'bg_{comp_key}']
            if bg_list:
                df_bg = make_correlation_df(bg_list, var_name='bg_corr')
    ...
    if results:
        ...
        df_res.to_csv(root_path/"figure/xi.csv", index=False)
